  ...
  covidInfo
    ...
    .then((userCovid) => {
      ...
      if (userCovid === null) {
        ...
        let countPositive = [];
        let covidPositive = String;
        userCovid.covidPositive.forEach((covidPositive) => {
            if(covidPositive.covidStatus === true){
                countPositive.push(covidPositive)
            }
        })
        if(countPositive.length === 0){
            covidPositive = 'chưa nhiễm'
        }
        else {
            covidPositive = 'Đã nhiễm' +  countPositive.length + "lần"
        }
        res.render("covidInfo", {
          requiredStatus: true,
          ...
          covidPositiveDate: userCovid.covidPositive[userCovid.covidPositive.length-1].date,
          tempToday:
            userCovid.heathHistories[
              userCovid.heathHistories.length - 1
            ].date.toDateString(),
          tempValue:
            userCovid.heathHistories[userCovid.heathHistories.length - 1].temp,
        });
      }
    });
};
    ...
    .then((userCovid) => {
      if (!userCovid || !userCovid._id) {
        ...
        const userCovid = new covidInfo({
          ...
          covidPositive: [{ covidStatus: covidStatus, date: covidDate }],
        });
        userCovid.save();
      } else {
        userCovid.heathHistories.push({
          date: new Date(),
          temp: req.body.tempStatus,
        });
